...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const attachments = [
      {
        filename: `Elya_Application_${(record.business_name || 'unknown').replace(/[^a-zA-Z0-9]/g, '_')}.pdf`,
        content: pdfBuffer,
      },
    ];
    ...
    const { error } = await resend.emails.send({
      ...
      subject: `Elya Application: ${record.business_name || 'Unknown Business'}`,
      html: `<h2>New Elya Partners Application Received</h2>
